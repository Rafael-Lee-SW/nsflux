...
from utils import load_model, load_data, process_format_to_response, process_to_format, error_format
...
class InferenceActor:
    async def __init__(self, config):
        ...
        self.model, self.tokenizer, self.embed_model, self.embed_tokenizer = load_model(config)
        ...
        while True:
            ...
            for http_query, future in batch:
                try:
                    ...
                    QU, KE, TA, TI = query_sort(user_input,
                                                model=self.model,
                    ...
                    if TA == "yes":
                        try:
                            docs, docs_list = execute_rag(QU, KE, TA, TI,
                                                          model=self.model,
                                                          tokenizer=self.tokenizer,
                                                          embed_model=self.embed_model,
                                                          embed_tokenizer=self.embed_tokenizer,
                            ...
                            output = generate_answer(QU, docs,
                                                     model=self.model,
                                                     tokenizer=self.tokenizer,
                                                     config=self.config)
