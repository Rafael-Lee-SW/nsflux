...
def first_llm(model, tokenizer, column_usage, user_query):
    ...
    너는 해운 회사의 데이터로 SQL 쿼리를 작성하는 데 도움을 주는 시스템이야. 사용자로부터 받은 질문을 분석하여, 필요한 경우에만 사용할 컬럼과 필터 조건, 집계 함수 및 정렬 조건을 추출해줘. 
    
    ### 참고 사항:
    1. 다음은 해운 회사 데이터의 메타데이터야. 테이블은 "revenue" 하나뿐이야.:
    "{column_usage}"
    2. 사용자가 입력한 질문을 분석하여 필요한 1) 필터 조건, 2) 집계 함수, 3) 정렬 기준, 4) SQL 쿼리, 5) SQL 쿼리에서 사용되는 모든 컬럼 리스트를 제공해줘. 1~3은 필요하지 않은 경우에는 추출해주지 않아도 돼.
    ...
    1) <filter/>출발지는 부산, 도착지는 일본, 날짜는 2024-08-01 이후 <filter>
    2) <aggregation/>화주(고객)별 매출액의 합계<aggregation>
    3) <order/>매출액 합계 기준 내림차순<order>
    4) <sql_query/>SELECT 
    5) <columns/>OUTPOL,OUTPOD,OUTBOR,OUTSTL,OUTSHP<columns>
    <column/>사용할 컬럼 (예: OUTDEM)
    2. 
