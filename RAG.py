...
# nest_asyncio를 적용하여 중첩 이벤트 루프 허용
import nest_asyncio
    ...
    if config.use_vllm:
        ...
        async def collect_outputs():
            ...
            async for output in model.generate(
                PROMPT, request_id="dummy_request_id", sampling_params=sampling_params
            ):
                current_output_str = str(output)
                # print("Yielding output:", output)
                # 이전 출력과 동일하면 루프 종료 (무한반복 방지)
                if current_output_str == prev_output_str:
                    print("Same output so that loop is done.")
                    break
                outputs.append(output)
                prev_output_str = current_output_str
                if (
                    hasattr(output, "finish_reason")
                    and output.finish_reason is not None
                ):
                    print("finish_reason is detected so that loop is done.")
                    break
            return outputs

        print("Where is the gracefully shut down the engine?1----------------1")

        # Use the current running loop (nest_asyncio is applied)
        try:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                print(
                    "Where is the gracefully shut down the engine?2---------------------2"
                )
                future = executor.submit(asyncio.run, collect_outputs())
                outputs = future.result(timeout=60)
        except Exception as ex:
            print("Error collecting async outputs:", ex)
            raise ex

        if not outputs:
            raise RuntimeError("No outputs were generated by the model.")
        print("Where is the gracefully shut down the engine?3----------------3")
        # print("Outputs :", outputs)
        # Instead of using only the first output, join all texts if needed.
        # full_text = "".join([out.text for out in outputs])
        # print("Full generated text:", full_text)
        # answer = "".join([getattr(out, "text", str(out)) for out in outputs])

        # 기존 코드:
        # answer = "".join(
        #     [getattr(out, "text", getattr(out, "prompt", str(out))) for out in outputs]
        # )

        # 수정된 코드: finished=True인 RequestOutput을 선택 (없으면 마지막 객체 사용)
        final_output = next(
            (o for o in outputs if getattr(o, "finished", False)), outputs[-1]
        )
        print("Finished=True / RequestOutput : ", final_output)
        answer = "".join(
            [getattr(comp, "text", "") for comp in getattr(final_output, "outputs", [])]
        )
        print("Final Answer of Token Problem Done")
        print("Final Answer of Token Problem : ", answer)
    else:
...
"""
    ...
    if config.use_vllm:
        ...
        # Use async generator with finish check
        import asyncio
        import concurrent.futures
        ...
        print("Inference steps : Colleting the token is done 1----------------1")

        # Use the current running loop (nest_asyncio is applied)
        try:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                print(
                    "Inference steps : Colleting the token is done2---------------------2"
                )
                future = executor.submit(asyncio.run, collect_outputs())
                outputs = future.result(timeout=60)
        except Exception as ex:
        ...
        if not outputs:
            raise RuntimeError("No outputs were generated by the model.")
        print("Inference steps : Colleting the token is done3----------------3 is done")
        ...
        print("Inference steps : Colleting the token is done : Generate the Answer is Done")
        print("Inference steps : Final Result : ", answer)
    else:
        print(">>> About to Tokenizer")
        input_ids = tokenizer(
            PROMPT, return_tensors="pt", truncation=True, max_length=4024
