...
"""
    ...
    if config.use_vllm:
        ...
        accepted_request_id = "dummy_request_id"

        # vllm을 통해 Model을 구동하여 Text를 생성한다.
        answer = collect_vllm_text(PROMPT, model, sampling_params, accepted_request_id)
    else:
...
@time_tracker
def collect_vllm_text(PROMPT, model, sampling_params, accepted_request_id):
    """
    ...
    # Run the asynchronous collection using a ThreadPoolExecutor.
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future = executor.submit(asyncio.run, collect_outputs())
        outputs = future.result(
            timeout=60
        )  # 매 loop를 통해 생성되는 결과값을 저장한다. -------- 최적화 여지 있음 ---------
